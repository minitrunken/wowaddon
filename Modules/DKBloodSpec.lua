...
function DKBloodSpec.Initialize()
    ...
    local function CanUsePestilence()
        local target = "target"
        local enemies = GetNumEnemiesWithinRange(10)
        local currentTime = GetTime()
        local canUsePestilence = UnitExists(target) and HasFrostFever(target) and HasBloodPlague(target) and enemies >= 2 and (AreBloodRunesAvailable() or AreDeathRunesAvailable()) and (currentTime - lastPestilenceTime) >= 30

        return canUsePestilence
    end


local function IsPestilenceUsed()
    -- Check if Pestilence was used in the combat log
    local timestamp, eventType, _, sourceGUID, sourceName, _, _, destGUID, destName, _, _, spellId = CombatLogGetCurrentEventInfo()
    if eventType == "SPELL_CAST_SUCCESS" and sourceGUID == UnitGUID("player") and spellId == 50842 then
        return true
    end
    return false
end

local function ResetPestilenceTimer()
    lastPestilenceTime = 0
    pestilenceReady = true
end



    local abilities = {
        ["OutBreak"] = { spellId = 77575, color = { r = 0.8, g = 0.1, b = 0.8 }, priority = 1, condition = function()
            local target = "target"
            return UnitExists(target) and not HasFrostFever(target) and not HasBloodPlague(target) and IsAbilityUsable(77575)
        end },
        ["Mind Freeze"] = { spellId = 47528, color = { r = 1, g = 0, b = 0 }, priority = 2, condition = function()
            local target = "target"
...
["Pestilence"] = { spellId = 50842, color = { r = 0.9, g = 0.9, b = 0 }, priority = 2, condition = function()
    local target = "target"
    local enemies = GetNumEnemiesWithinRange(10)
    local currentTime = GetTime()
    if (currentTime - lastPestilenceTime) >= 30 then
        pestilenceReady = true
    end

    local canUsePestilence = pestilenceReady and UnitExists(target) and HasFrostFever(target) and HasBloodPlague(target) and enemies >= 2 and IsAbilityUsable(50842) and (AreBloodRunesAvailable() or AreDeathRunesAvailable())
    
    if canUsePestilence then
        if IsPestilenceUsed() then
            lastPestilenceTime = currentTime
            pestilenceReady = false
        end
    end
    ...
    return canUsePestilence
        ...
        ["Rune Tap"] = { spellId = 48982, color = { r = 0, g = 1, b = 0 }, priority = 3, condition = function()
            local target = "target"
